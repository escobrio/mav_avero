...
def get_thrustvector_symb_base_frame(nozzle_index): 
    post_transform_sym = get_T_0E_symbolic()
    o_r_ey_sym = (nozzle_i_to_body(nozzle_index)@ post_transform_sym)[:3, 1]  # Assuming 1:3 corresponds to the first three rows
...
def main():
    nozzle_indices = [1, 2, 3]
    for nozzle_index in nozzle_indices:
        print("Nozzle Index: ", nozzle_index)
        print(nozzle_i_to_body(nozzle_index)) 
        # print(get_T_BE_symbolic(nozzle_index))
        print()
